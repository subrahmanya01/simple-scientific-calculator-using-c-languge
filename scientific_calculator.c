...
void error_shower(int error_code)
{
    ...
    switch(error_code)
    {
        case 1:fprintf(output,"Math error");
               exit(0);
        case 2:fprintf(output,"Syntax error");
               exit(0);
    }
}
...
int main()
{
	...
	while(!isempty_c(op_r))
     ...
     if(fix=='1')
     {
         switch(dec)
         {
             case 1:fprintf(output,"%.1lf",peek(&operand));
                    break;
             case 2:fprintf(output,"%.2lf",peek(&operand));
                    break;
             case 3:fprintf(output,"%.3lf",peek(&operand));
                    break;
             case 4:fprintf(output,"%.4lf",peek(&operand));
                    break;
             case 5:fprintf(output,"%.5lf",peek(&operand));
                    break;
             case 6:fprintf(output,"%lf",peek(&operand));
                    break;
             case 0:fprintf(output,"%.lf",peek(&operand));
                    break;
         }
     }
     else
         fprintf(output,"%lf",peek(&operand));
